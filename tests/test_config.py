...
class ConfigTests(unittest.TestCase):
    ...
    def test_loads_llama_cpp_embedding_configuration(self) -> None:
        gguf_config = VALID_CONFIG.replace(
            "embedding:\n  model_path: ./models/bge-m3",
            """embedding:
  backend: llama_cpp
  ...
  request_timeout_seconds: 45""",
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config_dir.mkdir()
            config_path = config_dir / "config.yaml"
            config_path.write_text(gguf_config, encoding="utf-8")
            ...
            self.assertEqual(config.embedding.base_url, "http://127.0.0.1:8081/v1")
            self.assertEqual(config.embedding.model, "bge-m3-q4")
    ...
    def test_rejects_unsupported_embedding_backend(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            config_dir = root / "config"
            config_dir.mkdir()
